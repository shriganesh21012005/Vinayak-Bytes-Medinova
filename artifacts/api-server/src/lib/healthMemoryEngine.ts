...
function mergeMedications(
  ...
  for (const med of newMeds) {
    if (!med.name || med.nameConfidence === "unverified") continue;
...
function buildCondensedProfile(memory: IHealthMemory): string {
  ...
  function medListOrNone(meds: IMedicationItem[]): string {
    if (!meds.length) return "None documented";
    return meds.map(m => {
      const parts = [m.name];
  ...
  const sections = [
    ...
    ``,
    `CURRENT MEDICATIONS`,
    medListOrNone(memory.currentMedications),
    ``,
...
export async function updateHealthMemory(
  ...
  const restrictionValues = [
    ...allCritical.filter(isRestriction),
  ];
  ...
  memory.allergies = mergeMemoryItems(
    memory.allergies,
    memorySignals.allergiesFound ?? [],
    recordOid,
    now
  );

  memory.chronicConditions = mergeMemoryItems(
  ...
  memory.surgeries = mergeMemoryItems(
    memory.surgeries,
    surgeryValues,
    recordOid,
    now
  ...
  memory.medicationRestrictions = mergeMemoryItems(
    memory.medicationRestrictions,
    restrictionValues,
    recordOid,
    now
  );

  memory.criticalEvents = mergeMemoryItems(
  ...
  memory.currentMedications = mergeMedications(
